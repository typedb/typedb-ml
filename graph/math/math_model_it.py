...
class ITGraknMathGraphFromQueryVariableGraphTuples(unittest.TestCase):
    def test_graph_is_built_as_expected(self):
        ...
        combined_graph = graph.load.build_graph_from_queries(query_sampler_variable_graph_tuples, mock_tx,
                                                             concept_dict_converter=model.concept_dict_to_grakn_math_graph)
        ...
        parent = neighbour.Role(parentship_exp, person_exp, 'parent')
        child = neighbour.Role(parentship_exp, person_exp, 'child')
